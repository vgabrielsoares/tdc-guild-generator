...
export function shouldCancelContract(reason: FailureReason): boolean {
  return reason === FailureReason.PICARETAGEM || 
 ...
 */
export function canContractReturnToAvailable(outcome: ContractResolution, reason?: FailureReason): boolean {
 ...
 */
export function shouldGetUnresolvedBonus(outcome: ContractResolution, reason?: FailureReason): boolean {
  if (outcome === ContractResolution.NAO_RESOLVIDO && reason) {
