...
export const useTimelineStore = defineStore('timeline', () => {
  ...
  const timelineStats = computed(() => {
    ...
    const todayEvents = events.filter(event => isSameDate(event.date, currentDate));
   ...
   */
  function createTimelineForGuild(guildId: string, startDate?: GameDate): GuildTimeline {
    const now = new Date();
  ...
  function removeEvent(eventId: string): boolean {
    ...
    const updatedEvents = currentTimeline.value.events.filter(event => event.id !== eventId);
