<template>
  <div
    class="guild-structure bg-gray-800 rounded-lg shadow-md p-6 border border-gray-700"
  >
      ...
      >
        ...
        >
          <InfoButton
            ...
            @open-help="$emit('open-help', 'guild-structure')"
          />
        </Tooltip>
      </h3>
      <Tooltip
    ...
    <div class="space-y-4">
      ...
      <div
        ...
        class="bg-amber-900 rounded-lg p-4 border border-amber-700"
      >
        <h4
          class="font-semibold text-amber-300 mb-2 flex items-center space-x-2"
        >
        ...
        <p class="text-amber-100 font-medium">
          Esta é uma Sede Matriz, com recursos e influência superiores às sedes
          comuns.
        </p>
      ...
      <!-- Descrição (se houver) -->
      <div
        v-if="guild.structure.description"
        class="bg-gray-700 rounded-lg p-4 border border-gray-600"
      >
        <h4
          class="font-semibold text-gray-300 mb-2 flex items-center space-x-2"
      ...
      <div class="bg-gray-700 rounded-lg p-4 border border-gray-600">
        ...
        >
          ...
          >
            <InfoButton
              help-key="guild-staff"
              @open-help="$emit('open-help', 'guild-staff')"
              button-class="text-xs"
            />
          </Tooltip>
        </h4>
        <p class="text-white font-medium">{{ guild.staff.employees }}</p>
        <p v-if="guild.staff.description" class="text-gray-300 text-sm mt-1">
          {{ guild.staff.description }}
        </p>

        <!-- Dica especial sobre dragões -->
        <div
          v-if="
            guild.staff.employees.includes('dragão') ||
            guild.staff.description?.includes('dragão')
          "
          class="mt-2 text-xs text-amber-300 bg-amber-900 bg-opacity-30 rounded p-2 border border-amber-700"
        >
          <span class="font-semibold">Easter Egg:</span> Um dragão disfarçado
          trabalha na sede! Isso acontece quando todas as rolagens de estrutura
          resultam em 23.
        </div>
      </div>
      ...
      <div class="bg-blue-900 rounded-lg p-4 border border-blue-700">
        <h4
          class="font-semibold text-blue-300 mb-2 flex items-center space-x-2"
        >
          ...
          >
            <InfoButton
              ...
              @open-help="$emit('open-help', 'settlement-types')"
              button-class="text-xs"
            />
          </Tooltip>
        </h4>
        ...
        <div class="mt-2 text-xs text-blue-200">
          ...
          <span
            v-if="
              guild.settlementType.includes('Lugarejo') ||
              guild.settlementType.includes('Povoado') ||
              guild.settlementType.includes('Aldeia')
            "
          >
            1d8 (resultados 1-8)
          </span>
          <span
            v-else-if="
              guild.settlementType.includes('Vilarejo') ||
              guild.settlementType.includes('Vila')
            "
          >
