<template>
  <div class="guild-display">
    ...
    <div v-else-if="guild" class="guild-content">
      <!-- Header da Guilda -->
      <div
        class="guild-header bg-gray-800 rounded-lg shadow-md p-6 mb-6 border border-gray-700"
      >
        <div class="flex items-center justify-between mb-4">
          ...
          <div class="flex-1 mr-4">
            <div v-if="!isEditingName" class="flex items-center group">
              ...
              <h1 class="text-3xl font-bold text-amber-400 mr-2">
                {{ guild.name }}
              </h1>
              <button
          ...
          <div class="flex space-x-2">
            <Tooltip
              content="Gera uma nova guilda com estrutura, relações e características completamente diferentes."
            ...
            <Tooltip
              content="Exporta todos os dados da guilda atual em formato CSV para backup ou compartilhamento."
            ...
            <Tooltip
              content="Remove a guilda atual. Esta ação não afeta o histórico de guildas salvas."
              title="Limpar Guilda"
            >
              <button
                @click="clearGuild"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <div class="bg-gray-700 rounded-lg p-3">
            <div
              class="font-semibold text-gray-300 flex items-center space-x-2"
            >
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <GuildStructure
          :guild="guild"
          @regenerate-structure="regenerateStructure"
          @open-help="openHelpModal"
        />
        <GuildRelations
          :guild="guild"
      ...
      <!-- Ações Rápidas -->
      <div
        class="mt-6 bg-gray-800 rounded-lg shadow-md p-6 border border-gray-700"
      >
        ...
        >
          ...
          >
            ...
            <span>{{
              isGuildInHistory ? "Já no Histórico" : "Salvar no Histórico"
            }}</span>
          </button>
          <button
            @click="clearGuild"
      ...
      <div
        v-if="guildStore.historyCount > 0"
        class="mt-6 bg-gray-800 rounded-lg shadow-md p-6 border border-gray-700"
      >
    ...
    <div v-else class="empty-state">
      <div
        class="text-center py-12 bg-gray-800 rounded-lg shadow-md border border-gray-700"
      >
        <BuildingStorefrontIcon class="w-24 h-24 text-gray-500 mb-4 mx-auto" />
        <h2 class="text-2xl font-semibold text-gray-300 mb-2">
          Nenhuma guilda gerada
        </h2>
        <p class="text-gray-400 mb-6">
          Configure o tipo de assentamento e gere uma nova guilda
        </p>
...
const regenerateGuild = async () => {
  if (guild.value?.locked) {
    toast.warning(
      "Esta guilda está bloqueada. Desbloqueie no histórico para permitir regeneração."
    );
    return;
  }
...
const regenerateStructure = async () => {
  if (guild.value?.locked) {
    toast.warning(
      "Esta guilda está bloqueada. Desbloqueie no histórico para permitir regeneração da estrutura."
    );
    return;
  }
...
const regenerateRelations = async () => {
  if (guild.value?.locked) {
    toast.warning(
      "Esta guilda está bloqueada. Desbloqueie no histórico para permitir regeneração das relações."
    );
    return;
  }
...
const regenerateGuildName = async () => {
  if (guild.value?.locked) {
    toast.warning(
      "Esta guilda está bloqueada. Desbloqueie no histórico para permitir regeneração do nome."
    );
    return;
  }
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Erro desconhecido ao salvar guilda";
  ...
  try {
    ...
    if (success) {
      ...
      const message = isNowLocked
        ? "Guilda bloqueada com sucesso!"
        : "Guilda desbloqueada com sucesso!";
