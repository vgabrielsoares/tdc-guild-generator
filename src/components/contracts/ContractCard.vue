    ...
    <div class="p-4 border-b border-gray-700">
      <div class="flex items-start justify-between">
        <div class="flex items-center gap-2">
          <ContractTooltip :content="getContractorTooltip()" :title="'Tipo de Contratante'"
          ...
          <InfoButton help-key="contract-contractors" @open-help="$emit('open-help', 'contract-contractors')"
    ...
    <div class="p-4 space-y-3">
      ...
      <div v-if="contract.objective">
        <div class="flex items-center gap-1 mb-1">
          <ContractTooltip :content="getObjectiveTooltip()" :title="'Objetivo do Contrato'">
            <p class="text-md font-medium text-gray-300">
              {{ contract.objective.category }}
            </p>
          </ContractTooltip>
          <InfoButton help-key="contract-objectives" @open-help="$emit('open-help', 'contract-objectives')"
            button-class="text-xs" />
        </div>
      ...
      <div v-if="contract.location" class="flex items-center gap-2 text-md text-gray-400">
        <ContractTooltip :content="getLocationTooltip()" :title="'Localização do Contrato'">
        ...
        <InfoButton help-key="contract-locations" @open-help="$emit('open-help', 'contract-locations')"
          button-class="text-xs" />
      </div>

      <!-- Valores e prazo -->
      <div class="flex items-center justify-between">
        <ContractTooltip :content="getDifficultyTooltip()" :title="'Valor e Dificuldade'"
          :difficulty="contract.difficulty" :has-modifiers="hasValueModifiers" :modifier-reasons="getModifierReasons()">
          <div class="flex items-center gap-1">
            <ContractValue :value="contract.value" :difficulty="contract.difficulty"
              :contractor-type="contract.contractorType" size="md" />
            <InfoButton help-key="contract-values" @open-help="$emit('open-help', 'contract-values')"
              button-class="text-xs" />
          </div>
        </ContractTooltip>

        <div v-if="contract.deadline.type !== 'Sem prazo'" class="text-right">
          <ContractTooltip :content="getDeadlineTooltip()" :title="'Prazo do Contrato'"
          ...
          <div v-if="daysUntilBreak !== null" class="text-sm mt-1" :class="
              daysUntilBreak <= 2
                ? 'text-red-400'
                : daysUntilBreak <= 5
                  ? 'text-orange-400'
      ...
      <div v-if="hasSpecialFeatures" class="flex items-center gap-2 pt-2 border-t border-gray-700">
        ...
        <div v-if="contract.status === ContractStatus.ANULADO"
          class="flex items-center gap-1 bg-gray-900/30 px-2 py-1 rounded-full text-md text-gray-400"
        ...
        <div v-if="contract.allies?.length"
          ...
          <span>{{ contract.allies.length }} aliado{{
        ...
        <div v-if="contract.additionalRewards?.length"
          ...
          <span>{{ contract.additionalRewards.length }} extra{{
...
function getDeadlineTooltip(): string {
  if (!props.contract.deadline || props.contract.deadline.type === DeadlineType.SEM_PRAZO) {
  ...
  tooltip += " Alguns prazos são arbitrários, outros têm janela de oportunidade específica.";
