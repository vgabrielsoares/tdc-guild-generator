<template>
  <div class="contract-list space-y-4">
    ...
    <div v-if="showFilters" class="flex flex-wrap gap-2">
      <button v-for="(status, index) in statusFilters" :key="status.value || `all-${index}`"
        @click="$emit('filter-status', status.value)" :class="[
    ...
    <div v-if="filteredContracts.length > 0" class="space-y-3">
      <div v-for="contract in paginatedContracts" :key="contract.id" class="transition-all duration-200">
        <ContractCard :contract="contract" :show-actions="showActions" @accept="handleAccept" @complete="handleComplete"
          @abandon="handleAbandon" @view-details="handleViewDetails" @open-help="$emit('open-help', $event)" />
      </div>
    </div>
    ...
    <div v-else-if="!isLoading" class="text-center py-8">
      ...
      <div class="flex items-center justify-center gap-2">
        <button v-if="canGenerate" @click="$emit('generate')"
...
interface Emits {
  ...
  regenerate: [];
  generate: [];
  'filter-status': [status: ContractStatus | null];
...
const filteredContracts = computed(() => {
  if (!props.activeStatusFilter) return props.contracts;
...
const statusFilters = computed(() => {
  ...
  return [
    ...
    },
    {
      ...
      count: statusCounts[ContractStatus.DISPONIVEL] || 0
    },
    {
      label: 'Aceito',
      value: ContractStatus.ACEITO,
      count: statusCounts[ContractStatus.ACEITO] || 0
    },
    {
      label: 'Em Andamento',
      value: ContractStatus.EM_ANDAMENTO,
      count: statusCounts[ContractStatus.EM_ANDAMENTO] || 0
    },
    {
      label: 'Concluído',
      value: ContractStatus.CONCLUIDO,
      count: statusCounts[ContractStatus.CONCLUIDO] || 0
    },
    {
      label: 'Falhou',
      value: ContractStatus.FALHOU,
      count: statusCounts[ContractStatus.FALHOU] || 0
    }
  ].filter(filter => filter.value === null || filter.count > 0);
});
