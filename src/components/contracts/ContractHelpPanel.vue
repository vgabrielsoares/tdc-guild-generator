<template>
    <div v-if="isOpen"
        ...
        <!-- Header fixo -->
        <div class="flex items-center justify-between p-4 border-b border-gray-600 flex-shrink-0">
            <h4 class="text-lg font-semibold text-amber-400 flex items-center space-x-2">
        ...
        <!-- Conteúdo com scroll -->
        <div v-if="helpData" class="flex-1 overflow-y-auto p-4 sm:p-4 space-y-3 custom-scrollbar">
            <!-- Descrição principal -->
            <div v-if="helpData.description" class="text-gray-300 text-sm leading-relaxed">
                {{ helpData.description }}
            </div>

            <!-- Seções de explicação -->
            <div v-if="helpData.sections" class="space-y-3">
                <div v-for="section in helpData.sections" :key="section.title" class="bg-gray-800 rounded-lg p-3">
                    <h5 class="font-medium text-amber-300 mb-2 flex items-center space-x-2">
            ...
            <!-- Dicas para mestres -->
            <div v-if="helpData.tips && helpData.tips.length > 0" class="bg-blue-900/30 rounded-lg p-3">
                ...
                <ul class="space-y-1">
                    <li v-for="tip in helpData.tips" :key="tip"
            ...
            <!-- Exemplos -->
            <div v-if="helpData.examples && helpData.examples.length > 0" class="space-y-2">
                <h5 class="font-medium text-green-300 flex items-center space-x-2">
                    <DocumentTextIcon class="w-4 h-4 flex-shrink-0" />
                    <span>Exemplos</span>
                </h5>
                <div v-for="example in helpData.examples" :key="example.title" class="bg-green-900/20 rounded-lg p-3">
                    <h6 class="font-medium text-green-200 mb-1 break-words">{{ example.title }}</h6>
                    <p class="text-green-100 text-sm leading-relaxed break-words">{{ example.description }}</p>
                </div>
            </div>
        </div>
