<template>
  <div
    class="roll-result p-4 rounded-lg border transition-all duration-300"
    :class="resultClasses"
  >
    <!-- Roll Header -->
    ...
    <div class="text-sm text-gray-400 space-y-1">
      <div
        v-if="(roll.individual && roll.individual.length > 1) || showDetails"
      >
        <span class="mr-1">Dados:</span>
        <span class="font-mono"
          >[{{ roll.individual?.join(", ") || "-" }}]</span
        >
      </div>
      ...
      <div v-if="roll.modifier !== undefined && roll.modifier !== 0">
        ...
        <span class="font-mono"
          >{{ roll.modifier > 0 ? "+" : "" }}{{ roll.modifier }}</span
        >
      </div>
    ...
    <div
      v-if="isNew && showAnimation"
      class="absolute inset-0 bg-amber-500 opacity-20 rounded-lg animate-pulse"
    ></div>
  </div>
</template>
