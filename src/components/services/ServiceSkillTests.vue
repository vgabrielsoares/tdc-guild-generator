<template>
  <div
    class="service-skill-tests bg-gray-800 rounded-lg border border-gray-700 p-6"
  >
    <!-- Cabeçalho dos Testes -->
    <div class="mb-6">
      <h3
        class="text-xl font-semibold text-amber-400 mb-2 flex items-center gap-2"
      >
    ...
    <div class="mb-6">
      ...
      <div class="w-full bg-gray-700 rounded-full h-2">
        <div
          ...
          :style="{
            width: `${(statistics.completedTests / statistics.totalTests) * 100}%`,
          }"
        ></div>
      </div>
    ...
    <div v-if="!skillsConfigured && !testStructure.completed" class="mb-6">
      <div class="bg-purple-900/30 border border-purple-700 rounded-lg p-4">
        ...
        <p class="text-purple-200 text-sm mb-4">
          Antes de iniciar os testes, configure qual perícia será usada para
          cada teste:
        </p>

        <div class="space-y-3 mb-4">
          <div
            v-for="(test, index) in testStructure.tests"
            :key="index"
          ...
          >
            ...
            <div class="flex-1">
              <input
                v-model="skillsSetup[index]"
                type="text"
                :placeholder="getSkillPlaceholder(index)"
                class="w-full bg-gray-700 border border-gray-600 rounded px-3 py-2 text-white focus:border-purple-500 focus:outline-none"
              />
            </div>
          </div>
        </div>
        ...
        <div class="flex gap-3">
          <button
            @click="configureSkills"
            :disabled="!allSkillsConfigured"
            class="px-4 py-2 bg-purple-600 text-white rounded hover:bg-purple-700 disabled:bg-gray-600 disabled:cursor-not-allowed transition-colors"
          >
            Iniciar Testes
          </button>
          <button
            @click="autoConfigureSkills"
    ...
    <div
      v-if="
        ...
        !skillsConfigured &&
        !testStructure.completed
      "
      class="mb-6"
    >
      <h4 class="text-lg font-medium text-amber-300 mb-3">
        Sugestões de Perícias
      </h4>
      <div class="grid gap-3">
        <div
          v-for="suggestion in skillSuggestions"
          :key="suggestion.skillName"
    ...
    >
      <div class="bg-blue-900/30 border border-blue-700 rounded-lg p-4">
        <h4 class="text-lg font-medium text-blue-300 mb-3">
          Teste {{ currentTestInfo.testNumber }} de
          {{ currentTestInfo.totalTests }}
        </h4>
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <p class="text-white font-bold text-lg">
              {{ skillsSetup[currentTestIndex] }}
            </p>
          </div>
          <div>
            <span class="text-blue-200 text-sm">ND do Teste:</span>
            <p class="text-white font-bold text-lg">
              {{ currentTestInfo.finalND }}
            </p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
          ...
          <div>
            ...
            <p class="text-white font-medium">
              {{ currentTestInfo.skillRequirement }}
            </p>
          </div>
        </div>
        ...
        <div class="flex gap-3 mb-4">
          <div class="flex-1">
            ...
            <input
              ...
              placeholder="Digite o resultado da rolagem"
              :disabled="isProcessing"
            />
          </div>
          <div class="flex flex-col justify-end">
            <button
              @click="processCurrentTest"
              :disabled="!isValidRoll || isProcessing"
        ...
        <div class="flex gap-2">
          <button
            @click="quickTest(true)"
            :disabled="isProcessing"
            class="px-3 py-1 bg-green-600 text-white text-sm rounded hover:bg-green-700 disabled:bg-gray-600 transition-colors"
          >
            Sucesso
          </button>
          <button
            @click="quickTest(false)"
            :disabled="isProcessing"
            class="px-3 py-1 bg-red-600 text-white text-sm rounded hover:bg-red-700 disabled:bg-gray-600 transition-colors"
          >
            Fracasso
          </button>
        </div>
    ...
    <div v-if="testStructure.tests.some((t) => t.completed)" class="mb-6">
      ...
      <div class="space-y-2">
        <div
          v-for="(test, index) in testStructure.tests"
          :key="index"
          v-show="test.completed"
          class="flex items-center justify-between bg-gray-900 rounded p-3"
          :class="{
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <div class="text-xs text-gray-400">
                {{ skillsSetup[index] || "Perícia não definida" }}
              </div>
            </div>
          </div>
          <div class="text-right">
            ...
            <div
              class="text-xs"
              :class="test.success ? 'text-green-400' : 'text-red-400'"
            >
    ...
    <div
      v-if="
        testStructure.completed && testStructure.outcome && formattedOutcome
      "
      class="mb-6"
    >
      <div
        ...
        :class="{
          'bg-green-900/30 border-green-500':
            formattedOutcome.statusClass === 'success',
          'bg-red-900/30 border-red-500':
            formattedOutcome.statusClass === 'failure',
        }"
      >
        <h4
        ...
        <div class="grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <span class="text-white ml-2">{{
              formattedOutcome.renownText
            }}</span>
          </div>
          <div>
            <span class="text-gray-400">Recompensa:</span>
            <span class="text-white ml-2">{{
              formattedOutcome.rewardText
            }}</span>
          </div>
        </div>
      </div>
    ...
    <div class="flex gap-3">
      ...
      <button
        @click="closeModal"
        class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors ml-auto"
      >
        Fechar
      </button>
    </div>
...
const allSkillsConfigured = computed(() => {
  return (
    skillsSetup.value.length === testStructure.value.totalTests &&
...
const progressBarColor = computed(() => {
  ...
  const successRate =
    statistics.value.successCount / statistics.value.completedTests;
  const failureRate =
    statistics.value.failureCount / statistics.value.completedTests;
...
function getSkillPlaceholder(testIndex: number): string {
  ...
  if (suggestions.length > 0) {
    return (
      suggestions[testIndex % suggestions.length]?.skillName ||
      `Perícia para teste ${testIndex + 1}`
    );
  }
...
function autoConfigureSkills() {
  ...
  for (let i = 0; i < testStructure.value.totalTests; i++) {
    if (suggestions.length > 0) {
      skillsSetup.value[i] =
        suggestions[i % suggestions.length]?.skillName || `Perícia ${i + 1}`;
    } else {
