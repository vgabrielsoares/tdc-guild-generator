...
describe("Proteção de Contratos dos Jogadores", () => {
  ...
  beforeEach(() => {
    ...
    mockContracts = [
      ...
      createMockContract({
        ...
        id: "player-accepted-1",
      }),
      createMockContract({
        status: ContractStatus.EM_ANDAMENTO,
        id: "player-progress-1",
      }),
      createMockContract({
      ...
      createMockContract({
        ...
        id: "resolved-by-others-1",
      }),
    ];
  });

  it("deve NÃO resolver contratos ACEITO (aceitos por jogadores)", () => {
    const originalContract = mockContracts.find(
      (c) => c.status === ContractStatus.ACEITO
    )!;
  ...
  it("deve APENAS resolver contratos ACEITO_POR_OUTROS (aceitos por NPCs)", () => {
    const npcContracts = mockContracts.filter(
    ...
    const processedNpcContracts = result.filter(
      (c) =>
    ...
    const playerContracts = result.filter(
      (c) =>
  ...
  it("deve preservar todos os outros status sem modificações", () => {
    const nonTargetContracts = mockContracts.filter(
      (c) => c.status !== ContractStatus.ACEITO_POR_OUTROS
    );
