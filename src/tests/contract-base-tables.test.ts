...
describe('Contract Base Tables - Issue 4.3', () => {
  describe('CONTRACT_DICE_BY_SIZE', () => {
    it('should match exactly the headquarters size specifications from .md', () => {
      ...
      expect(getContractDiceBySize("Pequeno e modesto (6m x 6m)")).toBe("1d6+1");
      expect(getContractDiceBySize("Pequeno e confortável (7,5m x 6m, +1 andar)")).toBe("1d6+1");
      expect(getContractDiceBySize("Mediano e comum (9m x 9m)")).toBe("1d8+2");
      expect(getContractDiceBySize("Mediano em dobro (10,5m x 9m, +1 andar)")).toBe("1d8+2");
      expect(getContractDiceBySize("Grande (12m x 12m)")).toBe("1d10+2");
      expect(getContractDiceBySize("Luxuosamente grande (12m x 12m, +2 andares)")).toBe("1d12+4");
      expect(getContractDiceBySize("Enorme (15m x 15m, +1 andar)")).toBe("1d20+4");
      expect(getContractDiceBySize("Enorme e confortável (15m x 15m, +2 andares)")).toBe("1d20+4");
      expect(getContractDiceBySize("Colossal (20m x 20m, +1 andar)")).toBe("1d20+8");
      expect(getContractDiceBySize("Colossal e primorosa (20m x 20m, +2 andares)")).toBe("1d20+10");
    });
  ...
  describe('CONTRACT_QUANTITY_TABLE', () => {
    ...
    it('should match specific values from .md', () => {
      const roll1to4 = CONTRACT_QUANTITY_TABLE.find(e => e.min === 1 && e.max === 4);
      expect(roll1to4?.result).toBe("1 contrato");

      const roll20 = CONTRACT_QUANTITY_TABLE.find(e => e.min === 20 && e.max === 20);
  ...
  describe('CROWD_REDUCTION_TABLE', () => {
    it('should match all crowd levels from .md', () => {
      ...
      expect(vazia?.result.reduction).toBe("Todos os contratos estão disponíveis");
  ...
  describe('CONTRACT_DEADLINE_TABLE', () => {
    ...
    it('should match specific deadline values from .md', () => {
      const roll1 = CONTRACT_DEADLINE_TABLE.find(e => e.min === 1 && e.max === 1);
      expect(roll1?.result.deadline).toBe("1d4 dias");

      const roll2 = CONTRACT_DEADLINE_TABLE.find(e => e.min === 2 && e.max === 2);
  ...
  describe('CONTRACT_VALUE_TABLE', () => {
    ...
    it('should match specific values from .md', () => {
      ...
      const roll1to8 = CONTRACT_VALUE_TABLE.find(e => e.min === 1 && e.max === 8);
      expect(roll1to8?.result).toBe(75);

      const roll9to15 = CONTRACT_VALUE_TABLE.find(e => e.min === 9 && e.max === 15);
      expect(roll9to15?.result).toBe(100);

      const roll100 = CONTRACT_VALUE_TABLE.find(e => e.min === 100 && e.max === 100);
  ...
  describe('CONTRACT_DISTANCE_TABLE', () => {
    ...
    it('should match specific distance values from .md', () => {
      const close = CONTRACT_DISTANCE_TABLE.find(e => e.min === 1 && e.max === 4);
      ...
      const far = CONTRACT_DISTANCE_TABLE.find(e => e.min === 20 && e.max === 20);
