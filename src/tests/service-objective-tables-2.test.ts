...
describe("Service Objective Tables - Part 2 (Issue 5.12)", () => {
  ...
  describe("Specific Table Content Validation", () => {
    ...
    it("AUXILIAR_OU_CUIDAR_TABLE should match .md content exactly", () => {
      ...
      expect(AUXILIAR_OU_CUIDAR_TABLE[0].result.complication).toBe("O responsável principal está ausente");
      
      expect(AUXILIAR_OU_CUIDAR_TABLE[6].result.action).toBe("Crianças");
      expect(AUXILIAR_OU_CUIDAR_TABLE[6].result.target).toBe("Cuidar de jardins ou estufas");
    });

    it("EXTRAIR_RECURSOS_TABLE should include all resource types from .md", () => {
      const resourceTypes = EXTRAIR_RECURSOS_TABLE.map(entry => entry.result.action);
    ...
    it("CONSTRUIR_CRIAR_OU_REPARAR_TABLE should include diverse construction types", () => {
      const constructionTypes = CONSTRUIR_CRIAR_OU_REPARAR_TABLE.map(entry => entry.result.action);
  ...
  describe("Resource Examples System", () => {
    ...
    it("RESOURCE_EXAMPLES should contain all resource types", () => {
      ...
      expect(RESOURCE_EXAMPLES["Componente mágico"]).toContain("cristais mágicos");
    });
  ...
  describe("Enhanced Generation Functions", () => {
    ...
    it("generateResourceExtractionDescription should handle normal resources", () => {
      ...
      expect(description).toMatch(/ferro|alumínio|manganês|magnésio|cobre|cromo|mercúrio|chumbo|estanho|ouro|prata|níquel|zinco|urânio/); // Algum exemplo de metal
    });
    ...
    it("generateCareDescription should adapt language for different targets", () => {
      const childCare = generateCareDescription("Crianças", "Cuidar de jardins ou estufas", "Há uma importante inspeção surpresa");
      expect(childCare).toMatch(/cuidar de|auxiliar/i);
      
      const adultHelp = generateCareDescription("Comerciantes", "Inventário de mercadorias", "Uma pessoa-chave insiste em mudar o plano");
      expect(adultHelp).toMatch(/auxiliar|ajudar/i);
    });

    it("generateEnhancedServiceObjective should work for new objective types", () => {
      const resourceObj = generateEnhancedServiceObjective(ServiceObjectiveType.EXTRAIR_RECURSOS);
      ...
      const careObj = generateEnhancedServiceObjective(ServiceObjectiveType.AUXILIAR_OU_CUIDAR);
      expect(careObj).not.toBeNull();
      expect(careObj?.type).toBe(ServiceObjectiveType.AUXILIAR_OU_CUIDAR);
      
      const constructObj = generateEnhancedServiceObjective(ServiceObjectiveType.CONSTRUIR_CRIAR_OU_REPARAR);
      expect(constructObj).not.toBeNull();
      expect(constructObj?.type).toBe(ServiceObjectiveType.CONSTRUIR_CRIAR_OU_REPARAR);
    });
  });
  ...
  describe("Integration with Existing System", () => {
    ...
    it("getThreeColumnTable should return tables for Issue 5.12 objective types", () => {
      expect(getThreeColumnTable(ServiceObjectiveType.AUXILIAR_OU_CUIDAR)).toBe(AUXILIAR_OU_CUIDAR_TABLE);
      expect(getThreeColumnTable(ServiceObjectiveType.EXTRAIR_RECURSOS)).toBe(EXTRAIR_RECURSOS_TABLE);
      expect(getThreeColumnTable(ServiceObjectiveType.CONSTRUIR_CRIAR_OU_REPARAR)).toBe(CONSTRUIR_CRIAR_OU_REPARAR_TABLE);
    });

    it("rollThreeColumnObjective should work with new tables", () => {
      const mockRoll = vi.fn().mockReturnValue(5);
      
      const careResult = rollThreeColumnObjective(AUXILIAR_OU_CUIDAR_TABLE, mockRoll, mockRoll, mockRoll);
      expect(careResult.action).toBe("Governo local");
      expect(careResult.target).toBe("Abater e preparar animais");
      expect(careResult.complication).toBe("A interferência de bestas atrapalha");
      
      const resourceResult = rollThreeColumnObjective(EXTRAIR_RECURSOS_TABLE, mockRoll, mockRoll, mockRoll);
  ...
  describe("Business Rules Validation", () => {
    ...
    it("should include complex complications reflecting real scenarios", () => {
      ...
      expect(complications.some(c => c.includes("clima") || c.includes("clima"))).toBe(true);
      expect(complications.some(c => c.includes("ferramenta") || c.includes("material"))).toBe(true);
    });
  });
  ...
  describe("Quality and Consistency", () => {
    ...
    it("should have consistent language and formatting", () => {
      ...
      allEntries.forEach(entry => {
        ...
        expect(entry.result.complication.trim()).toBe(entry.result.complication);
      });
    });

    it("should provide diverse and interesting combinations", () => {
      // Deve haver variedade suficiente para gameplay interessante
      const careTargets = new Set(AUXILIAR_OU_CUIDAR_TABLE.map(e => e.result.action));
      expect(careTargets.size).toBeGreaterThan(15); // Pelo menos 15 tipos diferentes
      
      const resourceTypes = new Set(EXTRAIR_RECURSOS_TABLE.map(e => e.result.action));
      expect(resourceTypes.size).toBeGreaterThan(8); // Pelo menos 8 tipos de recursos
      
      const constructionTypes = new Set(CONSTRUIR_CRIAR_OU_REPARAR_TABLE.map(e => e.result.action));
