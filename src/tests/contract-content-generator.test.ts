...
describe("Contract Content Generator (Issue 4.12)", () => {
  ...
  describe("Geração de Contratantes", () => {
    test("deve gerar contratante com tipo, nome e descrição", () => {
      const contract = ContractGenerator.generateBaseContract({ guild: mockGuild });
      
      const validTypes = [ContractorType.POVO, ContractorType.INSTITUICAO, ContractorType.GOVERNO];
    ...
    test("deve aplicar modificadores de relação corretamente", () => {
      ...
      for (let i = 0; i < 20; i++) {
        const contract = ContractGenerator.generateBaseContract({ guild: guildBadPopulation });
        contractsGenerated.push(contract.contractorType);
      }

      // Deve ter mais contratos do governo do que do povo
      const governmentContracts = contractsGenerated.filter(t => t === ContractorType.GOVERNO).length;
      const peopleContracts = contractsGenerated.filter(t => t === ContractorType.POVO).length;
    ...
    test("deve gerar contratante específico do governo quando aplicável", () => {
      ...
      const contract = ContractGenerator.generateBaseContract({ guild: guildGoodGovernment });
      ...
      if (contract.contractorType === ContractorType.GOVERNO) {
        ...
        expect(governmentTypes.some(type => 
  ...
  describe("Geração de Localidades", () => {
    ...
    test("deve gerar localizações variadas", () => {
      ...
      for (let i = 0; i < 30; i++) {
        const contract = ContractGenerator.generateBaseContract({ guild: mockGuild });
        const locationMatch = contract.description.match(/Local: ([^|]+)/);
        if (locationMatch) {
  ...
  describe("Pré-requisitos e Cláusulas", () => {
    test("deve incluir pré-requisitos para contratos de alto valor", () => {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        if (contract.value.experienceValue > 500 && contract.description.includes("Pré-requisitos:")) {
  ...
  describe("Coerência entre Elementos", () => {
    test("deve manter coerência entre dificuldade e valor", () => {
      ...
      if (contract.difficulty === ContractDifficulty.MORTAL) {
        expect(contract.value.experienceValue).toBeGreaterThan(contract.value.baseValue);
      }
    });
    ...
    test("deve aplicar modificadores de funcionários consistentemente", () => {
      ...
      const contract = ContractGenerator.generateBaseContract({ guild: guildExperienced });
  ...
  describe("Integração com Sistema Existente", () => {
    test("deve manter compatibilidade com geração múltipla", () => {
      const contracts = ContractGenerator.generateMultipleContracts({ guild: mockGuild });
