...
describe("Guild Structure Tables", () => {
  ...
  describe("Employees Table", () => {
    it("should roll on employees table", () => {
      ...
      expect(result.result).toMatch(
        /funcionário|membro|nobre|aventureiro|explorador|animal|ex-/
      );
    });
    ...
    it("should handle special result for 23 (dragon disguised)", () => {
      ...
      expect(
        EMPLOYEES_TABLE.find((entry) => entry.min <= 23 && entry.max >= 23)
      ).toBeDefined();
    });
  });
  ...
  describe("Settlement Dice Configuration", () => {
    it("should have proper dice configuration for structure", () => {
      ...
      expect(SETTLEMENT_DICE.structure["Lugarejo"]).toEqual({
        dice: "d8",
        modifier: 0,
      });
      expect(SETTLEMENT_DICE.structure["Metrópole"]).toEqual({
        dice: "d20",
        modifier: 8,
      });
    });
    ...
    it("should have proper dice configuration for visitors", () => {
      ...
      expect(SETTLEMENT_DICE.visitors["Lugarejo"]).toEqual({
        dice: "d8",
        modifier: 0,
      });
      expect(SETTLEMENT_DICE.visitors["Metrópole"]).toEqual({
        dice: "d20",
        modifier: 5,
      });
    });
...
describe2("Issue 3.2 - Guild Structure Generator", () => {
  describe2("Individual Generator Functions", () => {
    ...
    it2(
      "should generate headquarters characteristics based on size roll",
      () => {
        // Test different size rolls for different number of characteristics
        const smallResult = generateHeadquartersCharacteristics(
          5,
        ...
        const mediumResult = generateHeadquartersCharacteristics(
          12,
        ...
        const largeResult = generateHeadquartersCharacteristics(
          18,
  ...
  describe2("Complete Guild Generation", () => {
    it2("should generate a complete guild for a small settlement", () => {
      ...
      // Validate relations
      expect2(Object.values(RelationLevel)).toContain(
        result.guild.relations.government
      );
      expect2(Object.values(RelationLevel)).toContain(
        result.guild.relations.population
      );
      ...
      // Validate visitors
      expect2(Object.values(VisitorLevel)).toContain(
        result.guild.visitors.frequency
      );

      // Validate resources
      expect2(Object.values(ResourceLevel)).toContain(
        result.guild.resources.level
      );
    ...
    it2("should generate a complete guild for a large settlement", () => {
      ...
      expect2(result.logs).toBeInstanceOf(Array);
      expect2(
        result.logs.some((log) => log.includes("Calculated modifiers"))
      ).toBe(true);
    });
    ...
    it2("should apply custom modifiers correctly", () => {
      ...
      expect2(
        result.logs.some(
          (log) => log.includes("structure") && log.includes("+")
        )
      ).toBe(true);
    });
  });
    ...
    it2("should apply modifiers for experienced employees", () => {
      ...
      for (let i = 0; i < 10 && !foundExperienced; i++) {
        ...
        if (result.guild.staff.employees.includes("experiente")) {
          ...
          expect2(result.logs.some((log) => log.includes("modifiers"))).toBe(
            true
          );
        }
      }
  ...
  describe2("Headquarters (Sede Matriz) Functionality", () => {
    ...
    it2(
      "should apply +5 modifier for headquarters in structure generation",
      () => {
        ...
        for (let i = 0; i < 30 && !foundHeadquarters; i++) {
          ...
          if (result.guild.structure.isHeadquarters) {
            ...
            expect2(result.guild.structure.size).toBeTypeOf("string");
            expect2(result.guild.structure.characteristics).toBeInstanceOf(
              Array
            );
            expect2(result.guild.staff.employees).toBeTypeOf("string");
          }
    ...
    it2(
      "should include headquarters status in guild structure interface",
      () => {
        const config = { settlementType: SettlementType.CIDADE_GRANDE };
        const result = generateGuildStructure(config);
